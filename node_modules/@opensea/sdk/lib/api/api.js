...
class OpenSeaAPI {
    ...
    constructor(config, logger) {
        ...
        this.listingsAPI = new listings_1.ListingsAPI(fetcher);
        ...
        this.chainsAPI = new chains_1.ChainsAPI(fetcher);
        this.dropsAPI = new drops_1.DropsAPI(fetcher);
        this.transactionsAPI = new transactions_1.TransactionsAPI(fetcher);
        this.assetsAPI = new assets_1.AssetsAPI(fetcher);
        this.walletAuth = new walletAuth_1.WalletAuthAPI(fetcher);
    }
    /**
    ...
    async getBestListings(collectionSlug, limit, next, includePrivateListings, traits) {
        return this.listingsAPI.getBestListings(collectionSlug, limit, next, includePrivateListings, traits);
    }
    /**
     ...
     */
    async validateNFTMetadata(address, identifier, chain = this.chain, ignoreCachedItemUrls) {
        return this.nftsAPI.validateMetadata(address, identifier, chain, ignoreCachedItemUrls);
    }
    /**
    ...
    async getOffersByNFT(collectionSlug, identifier, limit, next) {
        return this.offersAPI.getOffersByNFT(collectionSlug, identifier, limit, next);
    }
    /**
     ...
     */
    async buildDropMintTransaction(slug, request) {
     ...
     */
    async buildCrossChainDropMintTransactions(slug, request) {
    ...
    async request(method, apiPath, body, headers, options) {
        return (0, rateLimit_1.executeWithRateLimit)(async () => {
            ...
            const wireBody = body == null || !shouldSnakeize ? body : (0, case_1.snakeizeKeysDeep)(body);
     ...
     * one verb.
     */
    camelizeResponseBody(raw, options) {
        return options?.camelizeResponse === false
            ? raw
            : (0, case_1.camelizeKeysDeep)(raw);
    }
     ...
     * @param options Request options like timeout and abort signal
     */
    async _fetch(url, method, headers, body, options) {
        const mergedHeaders = {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                const responseBody = (0, case_1.camelizeKeysDeep)(rawBody);
     ...
     */
    static async requestInstantApiKey(apiBaseUrl = constants_1.API_BASE_MAINNET) {
        const response = await fetch(`${apiBaseUrl}${(0, apiPaths_1.getInstantApiKeyPath)()}`, {
            method: "POST",
            headers: {
     ...
     */
    async _createRateLimitError(response) {
        ...
        error.responseBody =
            rawBody === undefined ? undefined : (0, case_1.camelizeKeysDeep)(rawBody);
        return error;
    }
}
