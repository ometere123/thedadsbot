...
class Seaport {
    ...
    ascendingAmountFulfillmentBuffer = 300, balanceAndApprovalChecksOnOrderCreation = true, conduitKeyToConduit, } = {}) {
        ...
        this.config = {
            ...
            conduitKeyToConduit: Object.fromEntries(Object.entries({
    ...
    async createOrder(input, accountAddress, exactApproval) {
        ...
        const { orderComponents: builtComponents, approvalActions } = await this._formatOrder(signer, offerer, Boolean(exactApproval), input);
        // Frozen because this same object is both exposed on the action and signed
        // below; see freezeOrderComponents.
        const orderComponents = (0, order_1.freezeOrderComponents)(builtComponents);
        ...
        return {
            ...
            executeApprovals: () => (0, usecase_1.executeApprovals)(actions),
        };
    }
    /**
    ...
    async createBulkOrders(createOrderInput, accountAddress, exactApproval) {
        ...
        for (const input of createOrderInput) {
            ...
            const { orderComponents, approvalActions } = await this._formatOrder(signer, offerer, Boolean(exactApproval), orderInput);
            // Frozen because these same objects are both exposed on the action and
            // signed below; see freezeOrderComponents.
            allOrderComponents.push((0, order_1.freezeOrderComponents)(orderComponents));
    ...
    async _formatOrder(signer, offerer, exactApproval, { conduitKey = this.defaultConduitKey, zone = ethers_1.ethers.ZeroAddress, zoneHash = ethers_1.ethers.ZeroHash, startTime = Math.floor(Date.now() / 1000).toString(), endTime = constants_1.MAX_INT.toString(), offer, consideration, counter, allowPartialFills, restrictedByZone, fees, domain, salt, }) {
        ...
        const considerationItemsWithFees = [
            ...
            ...(currencies.length
                ? (fees
                    ?.map(fee => (0, order_1.feeToConsiderationItem)({
                    fee,
                    ...
                    // range and is kept.
                    .filter(({ startAmount, endAmount }) => BigInt(startAmount) > 0n || BigInt(endAmount) > 0n) ?? [])
                : []),
        ];
    ...
    async fulfillOrder({ order, unitsToFill, offerCriteria = [], considerationCriteria = [], tips = [], extraData = "0x", accountAddress, conduitKey = this.defaultConduitKey, recipientAddress = ethers_1.ethers.ZeroAddress, domain, exactApproval = false, overrides, }) {
        ...
        if (!unitsToFill &&
            isRecipientSelf &&
            (0, fulfill_1.shouldUseBasicFulfill)(sanitizedOrder.parameters, totalFilled, tipConsiderationItems)) {
    ...
    async fulfillOrders({ fulfillOrderDetails, accountAddress, conduitKey = this.defaultConduitKey, recipientAddress = ethers_1.ethers.ZeroAddress, domain, exactApproval = false, overrides, }) {
        ...
        const fulfiller = await this._getSigner(accountAddress);
        const fulfillerAddress = await fulfiller.getAddress();
        const allOffererOperators = fulfillOrderDetails.map(({ order }) => this._getConduit(order.parameters.conduitKey));
        const fulfillerOperator = this._getConduit(conduitKey);
        ...
        const allTipItems = fulfillOrderDetails.map(({ tips = [] }) => tips.map(tip => ({
            ...(0, order_1.mapInputItemToOfferItem)(tip),
            recipient: tip.recipient,
        })));
    ...
    async getDomain(tag, index) {
        try {
            ...
            const totalDomains = await this.domainRegistry
                ...
                .catch(() => undefined);
            if (totalDomains === 0n) {
                throw new Error(`No domains are registered under tag ${tag}, so index ${index} is out of range.`);
            }
            throw error;
        }
    }
