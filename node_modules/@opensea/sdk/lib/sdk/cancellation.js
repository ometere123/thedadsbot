...
class CancellationManager {
    ...
    async cancelOrder({ order, orderHash, accountAddress, protocolAddress = utils_1.DEFAULT_SEAPORT_CONTRACT_ADDRESS, domain, }) {
        ...
        if (order) {
            ...
            if (!fetchedOrder.protocolAddress || !fetchedOrder.protocolData) {
                throw new Error(`Order ${orderHash} is missing protocolAddress or protocolData — cannot cancel.`);
            }
            (0, utils_2.requireValidProtocol)(fetchedOrder.protocolAddress);
    ...
    async cancelOrders({ orders, orderHashes, accountAddress, protocolAddress = utils_1.DEFAULT_SEAPORT_CONTRACT_ADDRESS, domain, overrides, }) {
        ...
        let firstOrderV2;
        let firstFetchedOrder;
        (0, utils_2.requireValidProtocol)(protocolAddress);
        const addProtocolAddress = (value) => {
            (0, utils_2.requireValidProtocol)(value);
        ...
        if (orders) {
            ...
            orderComponents = fetchedOrders.map(fetched => {
                if (!fetched.protocolAddress || !fetched.protocolData) {
                    throw new Error(`Order ${fetched.orderHash} is missing protocolAddress or protocolData — cannot cancel.`);
                }
        ...
        if (protocolAddresses.size > 1) {
            throw new Error("All orders in a cancelOrders batch must share the same protocolAddress. Cancel each protocol separately.");
        }
    ...
    async getOffererSignature(protocolAddress, orderHash, chain) {
        ...
        if (!("signer" in wallet)) {
            throw new Error("Please pass a Signer into this SDK to derive an offerer signature");
        }
        return wallet.signer.signTypedData({ chainId, name, version, verifyingContract: protocolAddress }, { OrderHash: [{ name: "orderHash", type: "bytes32" }] }, { orderHash });
    }
    /**
