...
const required=['README.md','SECURITY.md','.env.example','apps/dashboard/index.html','packages/core/src/intent-firewall.js','packages/cli/src/index.js','packages/agent/src/server.js','contracts/src/DelegatedMintWallet.sol'];
for(const f of required)if(!fs.existsSync(f)){console.error(`missing required file: ${f}`);process.exit(1);}
const skip=new Set(['node_modules','.git']);const js=[];
...
console.log(`repo check: ${required.length}/${required.length} required surfaces; ${js.length} JavaScript files syntax-clean`);
