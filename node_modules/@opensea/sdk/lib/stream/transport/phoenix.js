...
exports.PhoenixChannelsTransport = void 0;
/**
...
const defaultReconnectAfterMs = (tries) => DEFAULT_RECONNECT_BACKOFF_MS[tries - 1] ?? MAX_RECONNECT_BACKOFF_MS;
class PhoenixSubscription {
    ...
    rejoinTries = 0;
    rejoinTimer = null;
    constructor(topic, eventTypes, transport) {
...
class PhoenixChannelsTransport {
    endpoint;
    params;
    webSocketCtor;
    timeout;
    ...
    constructor(options) {
        ...
        const resolved = options.transport ??
            globalThis.WebSocket ??
            undefined;
        if (!resolved) {
            throw new Error("No WebSocket implementation found. Node 22+ and all browsers provide one globally; " +
                "on older runtimes pass `connectOptions.transport` (for example the `ws` package).");
        }
        this.webSocketCtor = resolved;
    }
    // ── URL ────────────────────────────────────────────────────────────
    endpointUrl() {
    ...
    // ── Subscriptions ──────────────────────────────────────────────────
    subscribe(topic, options, callbacks) {
        this.connect();
        ...
        const subscription = new PhoenixSubscription(topic, options?.eventTypes, this);
        if (callbacks) {
    ...
    sendJoin(subscription) {
        ...
        this.awaitReply(ref, (_response, status) => {
            if (status === "ok") {
                ...
                const error = new Error(`Subscribe to "${subscription.topic}" failed: ${status}`);
                for (const callbacks of subscription.callbackList) {
    ...
    scheduleRejoin(subscription) {
        ...
        this.log(`rejoining "${subscription.topic}" in ${delay}ms (attempt ${subscription.rejoinTries})`);
     ...
     */
    widenFilterIfNeeded(subscription, requested) {
        ...
        this.log(`widening the filter on "${subscription.topic}" to ${widened ? widened.join(", ") : "all events"}`);
        ...
        if (subscription.joinRef !== null) {
            this.push(subscription.joinRef, this.nextRef(), subscription.topic, PHX_LEAVE, {});
        }
        subscription.joined = false;
        this.sendJoin(subscription);
    }
    ...
    unsubscribeTopic(topic, onUnsubscribed) {
    ...
    removeHandler(topic, event, handler, onUnsubscribed) {
        const subscription = this.subscriptions.get(topic);
        if (!subscription) {
            return;
        }
    ...
    handleMessage(data) {
        ...
        if (frame.event === PHX_ERROR || frame.event === PHX_CLOSE) {
            ...
            subscription.joined = false;
            this.emitError(new Error(`Channel "${frame.topic}" received ${frame.event}`));
    ...
    awaitReply(ref, handler, onTimeout) {
        const timer = setTimeout(() => {
            this.pendingReplies.delete(ref);
            onTimeout(new Error(`Timed out after ${this.timeout}ms waiting for reply`));
        }, this.timeout);
    ...
    push(joinRef, ref, topic, event, payload) {
...
exports.PhoenixChannelsTransport = PhoenixChannelsTransport;
